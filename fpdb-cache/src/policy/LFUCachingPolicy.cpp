...
bool LFUCachingPolicy::lessValue (const std::shared_ptr<SegmentKey> &key1, const std::shared_ptr<SegmentKey> &key2) {
...
void LFUCachingPolicy::insert(int freq, const std::shared_ptr<SegmentKey> &key) {
...
void LFUCachingPolicy::onLoad(const std::shared_ptr<SegmentKey> &key) {
  // update keySet_
...
void LFUCachingPolicy::onRemove(const std::shared_ptr<SegmentKey> &key) {
...
LFUCachingPolicy::onStore(const std::shared_ptr<SegmentKey> &key) {
...
[[maybe_unused]] void LFUCachingPolicy::addEstimateCachingDecision(const std::shared_ptr<SegmentKey> &in,
                                                  const std::shared_ptr<SegmentKey> &out) {
  keysToReplace_.emplace(out);
  estimateCachingDecisions_.emplace(in, out);
}

void LFUCachingPolicy::removeEstimateCachingDecision(const std::shared_ptr<SegmentKey> &in) {
...
LFUCachingPolicy::getKeysetInCachePolicy() {
...
std::string LFUCachingPolicy::showCurrentLayout() {
  std::stringstream ss;
